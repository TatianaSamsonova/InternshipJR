...
public class ShipServiceImpl implements ShipService{
    ...
    public Ship createShip(Ship ship) {
        ...
        if(ship.getName() == null
            ...
            throw new BadRequestException("One of the Ship parameters is null");
    ...
    public Ship getShip(Long id) {
        ...
        if(!shipRepository.existsById(id)) throw new ShipNotFoundException("Ship hasn't been found");
    ...
    public void deleteById(Long id) {
        ...
        else throw new ShipNotFoundException("Ship hasn't been found");
    }

    @Override
    public Long checkAndParseId(String id) {

        if (id == null || id.equals("") || id.equals("0")) throw new BadRequestException("Incorrect id");
    ...
    private void checkShipParameters(Ship ship) {
        ...
        if (ship.getName() != null && (ship.getName().length() < 1 || ship.getName().length() > 50))
            throw new BadRequestException("Incorrect parameter Ship.name");

        if (ship.getPlanet() != null && (ship.getPlanet().length() < 1 || ship.getPlanet().length() > 50))
            throw new BadRequestException("Incorrect parameter Ship.planet");

        if (ship.getCrewSize() != null && (ship.getCrewSize() < 1 || ship.getCrewSize() > 9999))
            throw new BadRequestException("Incorrect parameter Ship.crewSize");

        if (ship.getSpeed() != null && (ship.getSpeed() < 0.01D || ship.getSpeed() > 0.99D))
            throw new BadRequestException("Incorrect parameter Ship.speed");

        if (ship.getProdDate() != null) {
            Calendar cal = Calendar.getInstance();
            cal.setTime(ship.getProdDate());
            if (cal.get(Calendar.YEAR) < 2800 || cal.get(Calendar.YEAR) > 3019)
                throw new BadRequestException("Incorrect parameter Ship.date");
        }
    }
